"""
Smart Phase 2: Optimized Portfolio Management with Feature Selection
Only using the most important features for trading decisions
"""
...
class SmartPortfolioEnv(gym.Env):
    """
    Smart portfolio environment with only the most important features
    Focused on price action, momentum, and sentiment
    """
    
    def __init__(self, df_dict, stock_list, initial_amount=1000000.0):
        super(SmartPortfolioEnv, self).__init__()
        ...
        # Smart action space: 7 actions for focused strategies
        self.action_space = spaces.Discrete(7)
        
        # Smart observation space: Only the most important features
        # 5 core features per stock + 8 market features = 5*45 + 8 = 233 features
    ...
    def _calculate_stock_scores(self, current_prices):
        ...
        for i, stock in enumerate(self.stock_list):
            if self.day < len(self.df_dict[stock]):
                ...
                momentum = data.get('close_momentum_5', 0)
                ...
                dma_dist = abs(data.get('dma_distance', 0))
                dma_score = max(0, 1 - dma_dist * 10)  # Closer to DMA = higher score
                
                # Combine scores
                scores[i] = momentum * 0.4 + rsi_score * 0.3 + volume_score * 0.2 + dma_score * 0.1
    ...
    def _get_obs(self):
        ...
        for i, stock in enumerate(self.stock_list):
            if self.day < len(self.df_dict[stock]):
                ...
                # Core features for this stock
                obs.extend([
                    data.get('close', 0) / 1000,  # Normalized price
                    data.get('volume', 0) / 10000000,  # Normalized volume
                    data.get('rsi_14', 50) / 100,  # Normalized RSI
                    data.get('close_momentum_5', 0),  # 5-day momentum
                    data.get('dma_distance', 0)  # DMA distance
                ])
            else:
...
def run_smart_phase2():
    """
    Run smart Phase 2 with feature selection
    """
    print("🚀 Smart Phase 2: Feature-Selected Portfolio Management")
    print("=" * 60)
    ...
    # Create environment
    print(f"\n🏗️ Creating smart environment...")
    env = SmartPortfolioEnv(df_dict, list(df_dict.keys()))
...
if __name__ == "__main__":
    success = run_smart_phase2()
    
    if success:
        print("\n🎉 Smart Phase 2 completed successfully!")
        print("✅ Feature selection working for >10% returns!")
    else:
