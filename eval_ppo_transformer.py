"""
PPO Transformer Model Evaluation Script

This script evaluates trained PPO models with Transformer architecture
and different feature combinations on out-of-sample data and provides comprehensive
...
from train_ppo_transformer import (
    PPOTrainer,
    CustomPortfolioEnv,
    CustomDataLoader,
    FeatureSelector,
...
class PPOTransformerEvaluator:
    """Comprehensive evaluation class for PPO Transformer models with feature combinations"""
    ...
    def __init__(self, model_path: str, config: Dict[str, Any], selected_features: List[str] = None):
        ...
        logger.info("PPO Transformer Evaluator initialized successfully")
    ...
    def _load_model(self):
        ...
        try:
            ...
            # Try to unpickle as a dict first
            try:
                model_state = pickle.loads(model_bytes)
                if isinstance(model_state, dict):
                    self.model_params = model_state.get('params')
                    self.training_config = model_state.get('config', {})
                    ...
                    # Assume it's just the params
                    from flax import serialization
                    self.model_params = model_state
                    self.training_config = {}
                    self.training_step = 0
                    self.selected_features = self.provided_features or []
                    self.feature_combination = 'unknown'
            except:
                # Try as serialized flax params
                from flax import serialization
                self.model_params = model_bytes
                self.training_config = {}
                self.training_step = 0
                self.selected_features = self.provided_features or []
                self.feature_combination = 'unknown'
            ...
            self.network = ActorCriticTransformer(
                ...
                hidden_dim=self.training_config.get('hidden_dim', 256),
                num_heads=self.training_config.get('num_heads', 4),
    ...
    def _get_policy_action(self, params, obs: jnp.ndarray, rng_key: jnp.ndarray, deterministic: bool = True):
        ...
        if deterministic:
            ...
            import distrax
            action_std = self.training_config.get('action_std', 0.3)
    ...
    def evaluate_model(self, num_episodes: int = 10, deterministic: bool = True) -> Dict[str, Any]:
        ...
        for episode in range(num_episodes):
            ...
            while not done and step_count < self.env.n_timesteps - 1:
                ...
                action, value = self._get_policy_action(
                    self.model_params, obs_batch, action_key, deterministic
                )
        ...
        results = {
            ...
            'algorithm': 'PPO with Transformer'
        }
    ...
    def generate_performance_report(self, save_path: Optional[str] = None) -> str:
        ...
        # Generate report
        report = f"""
PPO Transformer Portfolio Model Evaluation Report
=================================================
...
"""
    ...
    def create_visualizations(self, save_dir: Optional[str] = None):
        ...
        fig.suptitle(f'PPO Transformer Model Evaluation Results\n{self.feature_combination}',
        ...
        if save_dir:
            filename = f'ppo_transformer_evaluation_{self.feature_combination.replace("+", "_")}.png'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate PPO Transformer Model')
    ...
    parser.add_argument('--results_dir', type=str, default='evaluation_results/PPO_Transformer', help='Results directory')
    ...
    try:
        ...
        evaluator = PPOTransformerEvaluator(args.model_path, eval_config, selected_features=selected_features)
